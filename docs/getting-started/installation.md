---
description: Every way to install and start AgentHelm — release zip, from source with Aspire or dotnet run, and containers.
---
...
|---|---|:---:|:---:|---|
| [Release zip](#release-zip) | .NET 8 ASP.NET Core runtime | :material-check: | opt-in | Using AgentHelm |
| [From source with Aspire](#from-source-with-aspire) | .NET SDK 8.0.303+, Docker | :material-check: | :material-check: | Development, the full stack |
| [From source with `dotnet run`](#from-source-with-dotnet-run) | .NET SDK 8 | :material-check: | opt-in | Development without Docker |
| [Containers from GHCR](#containers-from-ghcr) | Docker | echo only | :material-check: | A quick look at the UI |
| [Containers built from source](#containers-built-from-source) | Docker | echo only | :material-check: | Testing the images |

> [!IMPORTANT]
> Real agents (Copilot CLI, Claude Code, Gemini CLI) run as **child processes of the Bridge**, with the Bridge's environment, credentials and file system. That is why every option that can drive real agents runs the Bridge directly on your machine, and why the container options only offer the built-in echo agent. [Deployment & topology](../configuration/deployment.md) explains the constraint in full.
...
The zip contains `bridge/`, `web/` and `echo-agent/` (the published applications), the two launchers, and `docker-compose.ghcr.yml`. The launchers start the Bridge in the background and the Web UI in the foreground; stopping the UI (++ctrl+c++) stops the Bridge too.

| Launcher variable | Default | Effect |
|---|---|---|
| `AGENTHELM_WEB_URLS` | `http://127.0.0.1:5200` | Where the Web UI listens. |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | `http://localhost:4318` | Where agents export OpenTelemetry (the [CopilotScope](../guide/copilotscope.md) collector by default). |
...
> [!TIP]
> The default launch profile uses HTTPS and the ASP.NET Core development certificate. If your browser complains about the certificate, run `dotnet dev-certs https --trust` once, or start with `dotnet run --project src/AgentHelm.AppHost --launch-profile http`.

If `dotnet run` fails with *"Aspire Workload has been deprecated"* or the Bridge cannot log in to PostgreSQL, see [Troubleshooting](../project/troubleshooting.md#aspire-workload-has-been-deprecated).
...
> [!CAUTION]
> The compose files publish their ports on **all** network interfaces and use a sample API token. On any machine that other devices can reach, bind the ports to loopback (`"127.0.0.1:5299:5199"`, `"127.0.0.1:5300:8080"`) and replace `dev-secret-123` in both `AgentHelm__ApiToken` and `Bridge__ApiToken`. The Bridge includes a terminal endpoint; see the [security model](../security/index.md#hardening-checklist).
...
If you enabled an API token, send it with the request: `curl -H "x-helm-token: <token>" …`. Then [run your first session](first-session.md).
