---
description: AgentHelm's GitHub Actions workflows, Dependabot, and the release process from tag to zip and container images.
---
...
> [!IMPORTANT]
> The `aspire` group updates `Aspire.*` **package references** only. `AgentHelm.AppHost.csproj` also pins `<Sdk Name="Aspire.AppHost.Sdk" Version="…" />`, which is an MSBuild SDK, not a package. The two must always move together — when an Aspire update arrives, bump the `Sdk` element by hand in the same pull request.
...
- [ ] This wiki describes the release — new settings, changed behaviour, removed [known issues](../project/troubleshooting.md#known-issues).
