---
description: Reference for the AgentHelm Bridge HTTP API — authentication, conventions, every endpoint, and the server-sent event streams.
---
...
> [!WARNING]
> The API is as powerful as the UI: it can run agent tools, execute shell commands and change files as the user running the Bridge. UI-only safeguards — such as the YOLO confirmation or the reject confirmation in the Changes tab — do not apply to API clients. See the [security model](../security/index.md).

## Conventions

- **Base URL:** `http://127.0.0.1:5199/api` (see [configuration](../configuration/index.md#bridge-settings)).
...
The event kinds are listed in [Events & transcript](events.md#live-events). A stream only carries what happens after you connect — load `GET /sessions/{id}` first for the transcript so far.
