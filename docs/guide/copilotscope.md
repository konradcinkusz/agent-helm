---
description: How AgentHelm works with CopilotScope — exporting agent telemetry to it and showing its quality scores next to a session.
---

# CopilotScope integration
...
The Bridge starts every agent with the environment variables from its [catalog entry](../configuration/agent-catalog.md). The preconfigured `copilot` entry switches on Copilot's OpenTelemetry export and points it at a local collector:
...
> [!NOTE]
> The match is **time-based and best-effort**. Scope identifies sessions by OpenTelemetry conversation ids that AgentHelm never sees, so it cannot know which Scope session is "this" one — it shows the ones that overlap in time and lets you judge. Several agents working at once will blur the picture. Exact correlation needs session tagging on the telemetry path, which is on the [roadmap](../project/roadmap.md) for both projects.
...
> [!NOTE]
> Despite its *OTel endpoint* label, the Pre-config field only changes where the **Bridge reads scores from**. Where agents **send** telemetry is decided by the `OTEL_*` variables above. With CopilotScope both are the same address, which is why one field is enough in the usual setup.
