---
description: Session history in PostgreSQL — what is saved and when, browsing archived transcripts, and resuming sessions through ACP session/load.
---
...
A session you started but never used is not saved. The agent's live "thinking" stream, tool-status updates, the terminal and pending permission requests are not part of the snapshot. See [Persistence](../architecture/persistence.md) for the table layout.
...
One row per session in the `helm_sessions` table of the `helmdb` database, with the snapshot as JSON. With Aspire the data is kept in the Docker volume `agenthelm-pgdata`, which survives restarts; removing the volume removes the history. [Persistence](../architecture/persistence.md) has the details.
