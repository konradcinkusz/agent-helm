...
> **Note:** macOS and BusyBox also ship a `script`, but not util-linux's: it rejects the options above. The Bridge asks the binary for its version once and uses PTY mode only for util-linux, so on those systems the terminal runs in pipe mode rather than failing to start.

Even in PTY mode, input is sent **a line at a time** from the input box. Full-screen programs (`vim`, `htop`, `less`) therefore cannot be used, and there is no way to send <kbd>Ctrl</kbd>+<kbd>C</kbd> — a command that never finishes keeps the shell busy until you delete the session. Prefer commands that exit on their own (for example `git --no-pager log`, `pytest -x`).
