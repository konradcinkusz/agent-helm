...
It then translates ACP's `session/update` kinds into `AgentEvent`s and ACP permission requests into `PermissionAsk`s; the tables are on the [Agent Client Protocol](Agent-Client-Protocol.md) page.
...
4. a row in the [agent catalog reference](Agent-Catalog.md#type-acp-or-copilot-sdk).
