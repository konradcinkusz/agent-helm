...
- One **child process** per session, started by `ProcessTransport` with the session's working directory (see [how agents are started](Agent-Catalog.md#how-an-agent-process-is-started)).
...
| Method | Handling |
|---|---|
| `session/request_permission` | Routed to the session's permission handler — the [policy engine](Permissions-and-Policies.md), then you. |
