...
3. builds the adapter through the [factory](Agent-Adapters.md#the-adapter-factory);
...
| `Policy` | `ask`, `auto_read`, `yolo` — see [Permissions & policies](Permissions-and-Policies.md) |
...
An append-only list of `ChatEntry(Time, Role, Text, Kind)`, guarded by a lock and copied on read. Roles are `user`, `assistant`, `tool` and `system`; the kinds and the exact audit texts are listed in [Events & transcript](Events-and-Transcript.md). The same entries are what [persistence](Persistence.md) stores.
