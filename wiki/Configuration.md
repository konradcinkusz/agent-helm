...
|---|---|---|
| `AgentHelm:Urls` | `http://127.0.0.1:5199` | Address(es) the Bridge listens on. Loopback only by default; widening it is a deliberate decision — read [Deployment & topology](Deployment-and-Topology.md#exposing-the-bridge) first. |
...
| `AgentHelm:Scope:BaseUrl` | `http://localhost:4318` | Base URL of the [CopilotScope](CopilotScope-Integration.md) API. `disabled` switches the integration off. |
| `AgentHelm:Agents` | four entries | The [agent catalog](Agent-Catalog.md). Read once at startup. |
| `ConnectionStrings:helmdb` | none | PostgreSQL connection string for [history](History-and-Resume.md). Without it the Bridge is memory-only. Aspire and the compose files set it for you. |
