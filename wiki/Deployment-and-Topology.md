...
The [security model](Security.md) lists the boundaries the Bridge holds and those it does not.
...
- **The GitHub Copilot SDK adapter** could talk to a shared `copilot --headless --port` server instead of spawning the CLI, but it is still a skeleton — see [Agent adapters](Agent-Adapters.md#the-copilot-sdk-adapter).
