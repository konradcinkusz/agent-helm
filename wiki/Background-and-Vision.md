...
3. **Security surface.** A browser GUI that executes tools on the machine is an attack vector — a web page can send requests to `localhost`. Loopback binding, a token and the permission gateway are the minimum, not options. See the [security model](Security.md).
