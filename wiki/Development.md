...
- Python 3 — optional, to check this wiki before a pull request ([Editing the wiki](Editing-the-Wiki.md)).
...
The [architecture overview](Architecture.md) explains how the pieces fit together.
...
- **New agent integrations go behind `IAgentAdapter`**, not into the session layer — see [Agent adapters](Agent-Adapters.md).
...
6. If behaviour or configuration changes, update this wiki in the same pull request ([Editing the wiki](Editing-the-Wiki.md)).
...
CI builds the solution and runs the tests on every pull request; pull requests that touch the wiki also run the wiki check. See [CI/CD & releases](CI-and-Releases.md).

## Reporting bugs and ideas

Use the issue templates. For a bug, include the steps (commands and clicks), expected and actual behaviour, `dotnet --version` and your OS, and the Bridge and Web logs (both log to stdout). Security problems go through [private reporting](Security.md#reporting-a-vulnerability), not issues.
