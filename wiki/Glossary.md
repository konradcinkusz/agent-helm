...
An open protocol, JSON-RPC 2.0 over stdio, between a client (an editor, or AgentHelm's Bridge) and a coding agent. See [agentclientprotocol.com](https://agentclientprotocol.com) and [Agent Client Protocol](Agent-Client-Protocol.md).

**Adapter**\
The Bridge component that drives one kind of agent protocol behind the `IAgentAdapter` interface: `AcpAdapter` today, `CopilotSdkAdapter` as a skeleton. See [Agent adapters](Agent-Adapters.md).
...
**Agent catalog**\
The list of agents under `AgentHelm:Agents` in the Bridge's configuration. See [Agent catalog](Agent-Catalog.md).

**Aspire**\
.NET Aspire, used by `AgentHelm.AppHost` to run PostgreSQL, the Bridge and the Web UI together during development.

**Audit entry**\
A `system` transcript entry recording a decision or action: permission requests and answers, policy changes, git accept/reject, handoffs. See [Events & transcript](Events-and-Transcript.md).
...
**Handoff**\
Continuing a conversation with another agent in a new session, with a summary prefilled — never sent automatically. See [Agent handoff](Agent-Handoff.md).

**History**\
Archived session snapshots in PostgreSQL. See [History & resume](History-and-Resume.md).
...
**Permission policy**\
How a session answers permission requests: `ask`, `auto_read` or `yolo`. Policies can only allow automatically. See [Permissions & policies](Permissions-and-Policies.md).
...
**Write-behind**\
Persisting changes asynchronously, about once a second, instead of on every change. See [Persistence](Persistence.md).
